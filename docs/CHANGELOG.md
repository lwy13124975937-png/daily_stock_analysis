...
- [修复] 稳健收益全市场预筛不再默认只深评前 30 只；真实 deep=30/60/120/240 审计后将生产预算提高到 240，并通过 `selection_mode`、未评估数量和 `is_exhaustive` 明确标注固定 shortlist，零候选结论不再冒充全市场穷举结果。
- [修复] 稳健收益深度评估改用互斥终态与安全 Provider 诊断，使用 CNInfo 行业 fallback 和 Provider 同期现金流比率打通普通企业真实评估；盘前/盘中价格时点改以最近已完成交易 session 校验，并严格区分合法零候选、部分降级与数据源不可用。
- [修复] 固定 Python lock 消费环境为 Ubuntu 24.04，并为 Docker 增加 Python 3.12/glibc 2.39 独立锁，避免 Longbridge SDK 与旧容器 ABI 不兼容。
- [改进] 为 Linux/Python 3.11 生产与 CI 路径增加完整传递依赖锁，并让每日任务、测试、发布和容器构建统一从 lock 安装。
- [测试] 增加 workflow 四模式、dirty source build identity、staging 故障注入、同日建议状态机、财务期间单位及公开 JSON HTML 注入回归。
- [修复] AI 建议回测将动作、情绪、持有结果和观望区间分开评估，按正式 A 股交易日和 post-close 锚点重算历史派生字段；历史 JSONL/manifest 损坏或缺失时停止覆盖，且同日重跑不再静默覆盖首条正式建议。
- [修复] 每日股票报告新增版本化结构化 JSON，coverage、日报有效性、建议回测和 Pages 优先消费同一 run 的结构化身份集合，删除按文件大小或 mtime 猜测有效产物的发布路径。
- [修复] 稳健收益按披露可得时点和已实施除权事件校验证据，金融行业缺少专用模型时 fail closed，并区分合法零候选与数据源失败；默认页面移除股息率反推观察价和未经证实的总回报措辞。
- [修复] Yahoo 基本面适配器改用显式 `as_of` 截止日并排除未来分红事件，同时锁定已通过字段契约测试的 Provider 版本，避免运行日期和依赖升级改变历史结果。
- [改进] GitHub Pages 收敛为单一 staging 构建，统一 sanitizer、CSS、build manifest、跨页 build_id、文件 hash、内链和敏感字段校验，避免旧账户页或半新半旧页面残留。
- [测试] 新增建议语义/否定句/节假日/历史迁移、稳健收益证据时点/分红实施/金融行业/线程隔离、持仓公开白名单及 Pages 原子构建和 HTML 安全回归。
- [修复] 对话消息读取统一按自增消息 ID 保持插入顺序，不再让同时间戳或系统时钟回拨打乱 user/assistant 次序。
- [修复] 报告 Markdown 抽屉在懒加载 chunk 临时失败后会轮换失败缓存，后续重新打开可恢复加载，同时保持错误被限制在抽屉内。
- [改进] 持仓列表构建脚本提供无网络副作用的标准 `--help`，本地与 CI 可先校验 CLI 契约而不会误触发 private holdings 下载。
- [修复] 稳健收益漏斗按结构化证据失败码统计数据不足，不再因同一证券同时命中硬排除规则而隐藏其证据缺口。

- [修复] GitHub Pages“稳健收益”改为覆盖全部沪深 A 股的两阶段规则筛选，不再以当前持仓作为候选池；页面公开全市场覆盖、基础预筛、深度评估和低风险合格数量，深评直接规范化财务摘要与实施分红记录以避免通用基本面预算耗尽造成全量“数据不足”，并由 HTML 检查阻止持仓口径回退；登录态 Web 的原持仓检查改名为“持仓稳健性”，避免同名误导。
- [修复] 静态持仓日报按账户渲染时读取原始 Markdown 的“未完成分析标的”，在账户摘要和持仓明细中展示失败原因，避免失败持仓被误标为 `AI摘要缺失`。
- [修复] GitHub Actions 日报覆盖检查改为只认可原始 Markdown 中的分析结果摘要、单项分析标题或“未完成分析标的”，避免页面兜底文本掩盖 stock/lof 持仓 code 漏分析。
- [新功能] Web 与 GitHub Pages 新增独立“稳健收益”模块，以现金流、盈利、已实施现金分红、回撤和波动作为风险硬门槛，覆盖沪深 A 股的分层筛选且不调用 LLM，也不返回持仓成本、数量、市值或盈亏。
